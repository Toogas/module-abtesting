...
class Edit extends \Toogas\AbTesting\Controller\Adminhtml\AbTest
{
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('toogas_abtesting_abtest', $model);

        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit A/B Test') : __('New A/B Test'),
            $id ? __('Edit A/B Test') : __('New A/B Test')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('A/B Tests'));
        $resultPage->getConfig()->getTitle()->prepend(
            $model->getId() ? __('Edit A/B Test %1', $model->getTitle()) : __('New A/B Test')
        );
        return $resultPage;
    }
}
